...
public final class ExtendedQueryMessageFlow {
     ...
     */
    public static Flux<BackendMessage> execute(Collection<Binding> bindings, Client client, PortalNameSupplier portalNameSupplier, String statement, boolean forceBinary) {
        ...
        if (bindings.size() == 1) {
            ...
            return client.exchange(toBindFlow(binding, portalNameSupplier, statement, forceBinary, true));
        } else {

            return client.exchange(Flux.fromIterable(bindings)
                .flatMap(binding -> toBindFlow(binding, portalNameSupplier, statement, forceBinary, false))
    ...
    private static Publisher<FrontendMessage> toBindFlow(Binding binding, PortalNameSupplier portalNameSupplier, String statement, boolean forceBinary, boolean inlineSync) {
